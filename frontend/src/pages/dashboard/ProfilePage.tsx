...
export const ProfilePage = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-16">
      {/* Header Banner */}
      <div className="relative rounded-3xl bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border border-slate-800 p-6 md:p-8 overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-gradient-to-tr from-indigo-500 via-violet-500 to-emerald-400 p-0.5 shadow-xl shrink-0">
              <div className="w-full h-full bg-slate-900 rounded-[14px] flex items-center justify-center text-xl md:text-2xl font-black text-white">
                {userInitials}
              </div>
            </div>
            <div>
          ...
          <div className="bg-slate-800/80 backdrop-blur-md border border-slate-700/60 rounded-xl px-4 py-3 text-right shrink-0">
            <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider block">Account Role</span>
            <span className="text-xs font-extrabold text-indigo-400 flex items-center justify-end gap-1.5 mt-0.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input 
                      ...
                      value={profile?.email || ''}
                      disabled
                      className="rounded-xl text-xs h-10 bg-slate-100/70 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 cursor-not-allowed pl-9"
                    />
                ...
                <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800/60">
                  ...
                  <div className="relative">
                    <Input 
                      ...
                      className="rounded-xl text-xs h-10 pl-9 font-medium border-indigo-200 dark:border-indigo-900/60 focus:ring-indigo-500/40"
                    />
        ...
        <div className="space-y-6">
          <Card className="bg-gradient-to-br from-indigo-900/40 via-slate-900 to-slate-900 border-indigo-500/30 text-white shadow-2xl rounded-2xl overflow-hidden relative">
            ...
            <CardHeader className="border-b border-indigo-500/20">
              <CardTitle className="text-xs font-bold uppercase tracking-wider text-indigo-300 flex items-center gap-2">
                <Sparkles className="w-3.5 h-3.5 text-indigo-400" /> Client Contact Card
              </CardTitle>
            ...
            <CardContent className="p-6 space-y-4 relative z-10">
              <div className="flex items-center gap-4 bg-slate-800/80 border border-slate-700/80 p-3.5 rounded-xl">
                ...
                <div className="overflow-hidden">
                  <p className="text-sm font-extrabold text-white truncate">{firstName} {lastName}</p>
                  <p className="text-[11px] text-indigo-300 font-medium truncate">{company?.name || 'SmartLedger Representative'}</p>
                </div>
              </div>

              <div className="space-y-2.5 text-xs bg-slate-900/80 p-4 rounded-xl border border-slate-800">
                <div className="flex items-center justify-between">
                  <span className="text-slate-400 text-[11px]">Direct Mobile:</span>
                ...
                <div className="flex items-center justify-between border-t border-slate-800 pt-2">
                  <span className="text-slate-400 text-[11px]">Email Support:</span>
                  <span className="font-semibold text-slate-200 truncate max-w-[180px]">{profile?.email}</span>
                </div>
                <div className="flex items-center justify-between border-t border-slate-800 pt-2">
                  <span className="text-slate-400 text-[11px]">Resolution Status:</span>
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold text-indigo-300 bg-indigo-950/80 px-2 py-0.5 rounded border border-indigo-800/60">
                    <CheckCircle2 className="w-3 h-3 text-emerald-400" /> Direct Support Active
                  </span>
                </div>
              </div>

              <div className="text-[10px] text-slate-400 leading-relaxed bg-indigo-950/30 border border-indigo-800/30 p-3 rounded-xl">
                💡 <strong className="text-slate-300">Client Tip:</strong> Adding your active mobile number ensures clients can reach out via phone/WhatsApp whenever they have billing questions or need receipt clarification.
              </div>
            </CardContent>
          </Card>
        </div>
