...
echo "4. Extract JWT and Refresh Tokens..."
JWT=$(echo "$LOGIN_OUT" | grep -o '"accessToken":"[^"]*' | cut -d'"' -f4)
REFRESH=$(echo "$LOGIN_OUT" | grep -o '"refreshToken":"[^"]*' | cut -d'"' -f4)
if [ -n "$JWT" ]; then
...
if [ -n "$REFRESH" ]; then
  echo "Refresh Token extracted: $REFRESH"
else
  echo "Failed to extract Refresh Token!"
fi
echo "----------------------------------------"

echo "5. Testing Protected Route /api/auth/me..."
